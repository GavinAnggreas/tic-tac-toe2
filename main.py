...
async def read_root():
    try:
        ...
        return HTMLResponse(content="<h1>Tic Tac Toe - Error loading page</h1><p>Please try refreshing the page.</p>", status_code=500)
...
def make_move_vercel(room_id: str, player_id: str, row: int, col: int):
    """Make move (row,col) using in-memory storage for Vercel"""
    if room_id not in games:
    ...
    if games[room_id].current_player != player_number:
        raise HTTPException(status_code=400, detail=f"It's not your turn! Current turn: {'Player X' if games[room_id].current_player == 1 else 'Player O'}")
    

    if row < 0 or row >= 3 or col < 0 or col >= 3 or games[room_id].board[row][col] != 0:
        raise HTTPException(status_code=400, detail="Invalid move! Cell is occupied or out of bounds.")

    if games[room_id].make_move(row, col):
        return {"success": True}
    else:
...
@app.post("/make-move")
async def make_move(room_id: str = Form(...), player_id: str = Form(...), row: int = Form(...), col: int = Form(...)):
    if IS_VERCEL:
        return make_move_vercel(room_id, player_id, row, col)
            ...
            while True:
                ...
                if message["type"] == "make_move":
                    ...
                    if games[room_id].current_player != player_number:
                        error_message = {
                            "type": "error",
                            "message": f"It's not your turn! Current turn: {'Player X' if games[room_id].current_player == 1 else 'Player O'}"
                        }
                    ...
                    if games[room_id].make_move(row, col):
                        ...
                        # If vs computer and it's computer's turn, make a random move
                        if not games[room_id].game_over and games[room_id].current_player == 2:
                            ...
                            if computer_count > 0:
                                import random
                                empty_cells = [(r, c) for r in range(3) for c in range(3) if games[room_id].board[r][c] == 0]
                                if empty_cells:
                                    ai_row, ai_col = random.choice(empty_cells)
                                    games[room_id].make_move(ai_row, ai_col)
